...
import { CheckCircle2, XCircle, Upload, FileSpreadsheet, Trash2, Wallet, Loader2 } from "lucide-react"
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        const result = response.data as BulkDepositResponse
        ...
        if (result.results) {
          const resultMap = new Map(result.results.map((r: any, idx: number) => [idx, r]))
    ...
    <Card className="border-0 shadow-lg rounded-2xl overflow-hidden">
      ...
      <CardHeader className="pb-4">
        <div className="flex items-center gap-3">
          <div className="icon-container bg-gradient-to-br from-[hsl(178,72%,28%)] to-[hsl(186,70%,38%)] text-white">
            <Wallet className="h-5 w-5" />
          </div>
          <div>
            <CardTitle className="text-xl">إيداع جماعي</CardTitle>
            <CardDescription>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <Select value={currency} onValueChange={setCurrency} disabled={processing}>
            <SelectTrigger className="h-12 bg-white border-0 focus:ring-2 focus:ring-primary/20 rounded-xl text-right">
        ...
        {/* File Upload Area */}
        <div
          className={`border-2 border-dashed rounded-xl p-8 text-center transition-colors ${file ? 'border-primary/50 bg-primary/5' : 'border-muted-foreground/20 hover:border-primary/50'}`}
            ...
            {file ? (
              ...
              <>
                ...
                <div className="text-muted-foreground">
                  ...
                  <Button variant="link" className="px-1 text-primary" onClick={() => fileInputRef.current?.click()}>
                    تصفح
                  </Button>
                </div>
        ...
        {summary && (
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="bg-emerald-500/10 p-4 rounded-xl text-center border border-emerald-500/20">
              ...
              <div className="text-2xl font-bold text-emerald-700">{summary.success}</div>
            </div>
        ...
        {data.length > 0 && (
          <div className="border rounded-xl overflow-hidden">
            <div className="bg-muted/30 px-4 py-3 text-sm font-medium border-b flex justify-between items-center">
        ...
        <Button
          ...
          className="w-full h-12 text-base font-semibold bg-gradient-to-r from-[hsl(178,72%,28%)] to-[hsl(186,70%,38%)] hover:from-[hsl(178,72%,23%)] hover:to-[hsl(186,70%,33%)] transition-all duration-300 shadow-lg hover:shadow-xl rounded-xl"
        >
          {processing ? (
